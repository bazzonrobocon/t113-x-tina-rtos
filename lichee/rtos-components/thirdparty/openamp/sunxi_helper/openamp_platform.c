...
#ifdef OPENAMP_DBG
static void dump_rsc(void *rsc_table)
{
	...
	for (i = 0; i < r_table->num; i++) {
		...
		switch (lrsc_type) {
		case RSC_VDEV: {
				struct fw_rsc_vdev *vdev_rsc = (struct fw_rsc_vdev *)rsc_start;
				openamp_dbg("\t VDEV:\r\n");
				openamp_dbg("\t      type:%d,id:%d\r\n", vdev_rsc->type, vdev_rsc->id);
				openamp_dbg("\t      notifyid:%d\r\n", vdev_rsc->notifyid);
				openamp_dbg("\t      dfeatures:0x%x\r\n", vdev_rsc->dfeatures);
				openamp_dbg("\t      gfeatures:0x%x\r\n", vdev_rsc->gfeatures);
				openamp_dbg("\t      vring[0]:da:0x%x, align:%d, num:%d\r\n",
											vdev_rsc->vring[0].da, vdev_rsc->vring[0].align,
											vdev_rsc->vring[0].num);
				openamp_dbg("\t      vring[1]:da:0x%x, align:%d, num:%d\r\n",
											vdev_rsc->vring[1].da, vdev_rsc->vring[1].align,
											vdev_rsc->vring[1].num);
			}
			break;
		case RSC_CARVEOUT: {
				struct fw_rsc_carveout *carveout_rsc = (struct fw_rsc_carveout *)rsc_start;
				openamp_dbg("\t CARVEOUT: %s\r\n", carveout_rsc->name);
				openamp_dbg("\t      da:0x%x, pa:0x%x, len:0x%x\r\n",
								carveout_rsc->da, carveout_rsc->pa, carveout_rsc->len);
			}
			break;
		case RSC_DEVMEM: {
				struct fw_rsc_devmem *devmem_rsc = (struct fw_rsc_devmem *)rsc_start;
				openamp_dbg("\t DEVMEM: %s\r\n", devmem_rsc->name);
				openamp_dbg("\t      da:0x%x, pa:0x%x, len:0x%x\r\n",
								devmem_rsc->da, devmem_rsc->pa, devmem_rsc->len);
			}
			break;
		case RSC_TRACE: {
				struct fw_rsc_trace *trace_rsc = (struct fw_rsc_trace *)rsc_start;
				openamp_dbg("\t TRACE: %s\r\n", trace_rsc->name);
				openamp_dbg("\t      da:0x%x, len:0x%x\r\n", trace_rsc->da, trace_rsc->len);
			}
			break;
		default:
			openamp_dbg("\t Undown:%d\r\n", lrsc_type);
		}
	}
}
#endif
...
static struct openamp_sunxi_rproc *openamp_sunxi_rproc_init(int ops_id)
{
	...
	if(ops_id >= sunxi_rproc_impls_size) {
		openamp_err("remoteproc_init failed\r\n");
		goto free_rproc;
	}
	impl = &(sunxi_rproc_impls[ops_id]);
...
struct remoteproc *openamp_sunxi_create_rproc(int rproc_id, int ops_id, int rsc_id)
{
	int ret;
	void *rsc_table = NULL;
	int rsc_table_size;
	...
	ret = resource_table_init(rsc_id, &rsc_table, &rsc_table_size);
	if (ret < 0) {
		openamp_err("resource_table_init failed\n");
		goto out;
	}
	...
	rproc = openamp_sunxi_rproc_init(ops_id);
	if(rproc == NULL)
		goto out;

	/*  Mmap resource table, so that remoteproc can use it. */
	openamp_dbg("start map resource table\r\n");
	ret = mem_va_to_pa((unsigned long)rsc_table, &table_pa, &attribute);
	if(ret != 0) {
	...
	ret = remoteproc_set_rsc_table(&rproc->inst, (struct resource_table *)rsc_table,
					rsc_table_size);
	if(ret != 0) {
		openamp_err("Failed to parse resource_table %p, ret=%d.\n", rsc_table, ret);
		goto remove_rproc;
	}
...
{
	struct openamp_sunxi_rproc *platform_rproc = metal_container_of(rproc,
			struct openamp_sunxi_rproc, inst);
	return platform_rproc->id;
}
		...
		rpmsg_ns_bind_cb ns_bind_cb)
{
	...
	rvdev_id = openamp_sunxi_get_rproc_id(rproc);

	/* Get shared memory resource */
	rvdev_shm = resource_table_get_rvdev_shm_entry(rproc->rsc_table, rvdev_id);
	if(!rvdev_shm) {
	...
	openamp_dbg("shm_io va:0x%p pa:0x%lx\r\n", shm_io->virt, *(shm_io->physmap));
	...
	if (!vdev) {
		openamp_err("Failed to Create virtio device\r\n");
		goto free_rvdev;
	}
	...
	/*
	 * We use the rproc id as the global unique rpmsg vdev id. Because the
	 * id of each rpmsg vdev in each rproc is always 0, we cannot use it as
	 * the global id. Since each rproc has only one vdev used for rpmsg,
	 * therefore we use the rproc id instead.
	 *
	 * e.g.
	 *   rproc0 (rproc id: 0)
	 *     |--- rpmsg_vdev (vdev_id: 0) (global rpmsg_vdev id: 0)
	 *     |--- another vdev (vdev_id: 1) (not rpmsg_vdev, has not global rpmsg_vdev id)
	 *     |--- other vdevs... (have not global rpmsg_vdev ids)
	 *
	 *   rproc1 (rproc id: 1)
	 *     |--- rpmsg_vdev (vdev_id: 0) (global rpmsg_vdev id: 1)
	 *     |--- another vdev (vdev_id: 1) (not rpmsg_vdev, has not global rpmsg_vdev id)
	 *     |--- other vdevs... (have not global rpmsg_vdev ids)
	 */
	rvdev->id = rvdev_id;
...
{
	struct rpmsg_virtio_device *rvdev = metal_container_of(rpmsg_dev,
			struct rpmsg_virtio_device, rdev);
	struct openamp_sunxi_rvdev *sunxi_rvdev = metal_container_of(rvdev,
			struct openamp_sunxi_rvdev, inst);
	return sunxi_rvdev->id;
}
