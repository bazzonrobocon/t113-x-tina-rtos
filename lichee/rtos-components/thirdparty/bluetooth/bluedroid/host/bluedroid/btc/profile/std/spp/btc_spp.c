...
int bta_co_rfc_data_incoming(void *user_data, BT_HDR *p_buf)
{
    ...
    if (spp_local_param.spp_mode == XR_SPP_MODE_CB) {
        ...
        if (rx_len == 0) {
            BTC_TRACE_DEBUG("%s data post! %d, %lu", __func__, slot->rfc_handle, (unsigned long)rx_len);
            status = btc_transfer_context(&msg, &p_data, sizeof(tBTA_JV), NULL);
            assert(status == BT_STATUS_SUCCESS);
        } else if (fixed_queue_length(slot->rx.queue) > 2) {
            BTC_TRACE_DEBUG("%s data post stop! %d %lu", __func__, slot->rfc_handle, (unsigned long)fixed_queue_length(slot->rx.queue));
...
// fix unused-function
__attribute__((__unused__)) static ssize_t spp_vfs_write(int fd, const void * data, size_t size)
{
    assert(data != NULL);
    errno = 0;
    if (size == 0) {
    ...
    while (1) {
        ...
        if ((slot = spp_local_param.spp_slots[serial]) != NULL) {
            ...
            if (!enqueue_status) {
                BTC_TRACE_DEBUG("%s tx_len:%lu, fd:%d\n", __func__, (unsigned long)fixed_queue_length(slot->tx.queue), fd);
                osi_mutex_unlock(&spp_local_param.spp_slot_mutex);
...
// fix unused-function
__attribute__((__unused__)) static int spp_vfs_close(int fd)
{
    errno = 0;
    if (!is_spp_init()) {
        BTC_TRACE_ERROR("%s SPP have not been init\n", __func__);
...
// fix unused-function
__attribute__((__unused__)) static ssize_t spp_vfs_read(int fd, void * dst, size_t size)
{
    assert(dst != NULL);
    errno = 0;
    if (!is_spp_init()) {
