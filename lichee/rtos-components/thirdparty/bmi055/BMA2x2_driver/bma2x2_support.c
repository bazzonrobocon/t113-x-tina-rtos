 ...
 *	\return: communication result
 */
static int cmd_bma2x2_data_readout_template(int argc, const char **argv)
{
 ...
 *-------------------------------------------------------------------------*/
	/* spi bus */
	bma2x2.dev_addr = SPI_PORT;
	com_rslt = bma2x2_init(&bma2x2);
	if (com_rslt != 0) {
		printf("bma2x2_init return error\n");
		SPI_routine_deinit();
		return 0;
	} else
		printf("bma2x2 chip_id 0x%x\n", bma2x2.chip_id);
 ...
 *	required for read/write operations*/
	...
	if (com_rslt != 0) {
		printf("bma2x2_set_power_mode return error\n");
		SPI_routine_deinit();
		return 0;
	}
/*	Note:
...
*---------------------------------------------------------------------------*/
	...
	if (com_rslt != 0) {
		printf("bma2x2_set_bw return error\n");
		SPI_routine_deinit();
		return 0;
	}
	...
	if (com_rslt != 0) {
		printf("bma2x2_get_bw return error\n");
		SPI_routine_deinit();
		return 0;
	} else
...
*---------------------------------------------------------------------*/
		...
		printf("bma2x2 accel accel_x %d accel_y %d accel_z %d\n",
				accel_x_s16, accel_y_s16, accel_z_s16);

		printf("bma2x2 accel sample_xyz.x %d sample_xyz.y %d sample_xyz.z %d\n",
					sample_xyz.x, sample_xyz.y, sample_xyz.z);

		printf("bma2x2 accel sample_xyzt.x %d sample_xyzt.y %d sample_xyzt.z %d sample_xyzt.temp %d\n",
					sample_xyzt.x, sample_xyzt.y, sample_xyzt.z, sample_xyzt.temp);

		BMA2x2_delay_msek(500);
	}
...
FINSH_FUNCTION_EXPORT_CMD(cmd_bma2x2_data_readout_template, bmi_accel_test, spi imu bmi055 accel test)
 ...
 *--------------------------------------------------------------------------*/
	...
	hal_gpio_pinmux_set_function(ACCEL_CS, GPIO_MUXSEL_OUT);
	hal_gpio_set_data(ACCEL_CS, GPIO_DATA_HIGH);
...
s8 BMA2x2_SPI_bus_read(u8 dev_addr, u8 reg_addr, u8 *reg_data, u8 cnt)
{
	...
	tr.rx_buf = &array[BMA2x2_INIT_VALUE + 1];
	tr.rx_len = cnt;
	tr.tx_buf = &array[BMA2x2_INIT_VALUE];
	tr.tx_len = 1;
	tr.dummy_byte = 0;
	tr.tx_single_len = 1;
	...
	hal_gpio_set_data(ACCEL_CS, GPIO_DATA_LOW);
