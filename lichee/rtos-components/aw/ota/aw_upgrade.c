...
static int upgrade_slice_sub_action(char *source_file, unsigned char *source_buffer, char *dest_file, int offset, int slice_size, int action)
{
	int source = -1, dest = -1;
	...
	if (source_buffer) {
		...
		if (-1 == stat(source_file, &statbuff)) {
			...
			if (statbuff.st_size != slice_size) {
				OTA_ERR("size error, file size is %ld, not %d\n", statbuff.st_size, slice_size);
				ret = -1;
				goto upgrade_slice_sub_action_out;
			}
		}
...
int upgrade_slice(char *source_file, unsigned char *source_buffer, char *dest_file, int offset, int slice_size)
{
...
int aw_upgrade_slice(char* target, uint8_t* buffer, uint32_t offset, uint32_t size, uint32_t flag)
{
	int ret;
	char *target_file;
