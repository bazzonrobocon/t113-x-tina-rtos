...
__attribute__((__unused__)) static btmg_callback_t *btmg_callback_get(btmg_cb_id_t cb_id) // fix unused-function
{
    if (cb_id < 0 || cb_id >= CB_MAX) {
        return NULL;
    }
...
//If the saved address is not found, a new address is generated.
    ...
    if (pFile) {
        ...
        for (i = 0; i < 6; i++) {
            sprintf((char *)&(tmp_mac[i * 3]), "%02x:", mac[i]);
        }
         //Saved new address.
...
btmg_err btmg_adapter_get_name()
{
    ...
    return xr_bt_dev_get_device_name();
}
