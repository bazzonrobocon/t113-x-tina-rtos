...
int btcli_gatts_attributedata_mtu_exchanged(int conn_id)
{
    ...
    if (conn_id >= 0 && conn_id < MAX_LE_CLIENT_CONN) {
        ...
        ret = btmg_le_gatt_mtu_exchange(conn_id);
        if (ret) {
            CMD_ERR("exchange att mtu failed!");
            connection->att_mtu_max = 20; // att data len dafault
        }
    }
    return ret;
}

void btcl_gatts_attributedata_set_max_mtu(int conn_id, int mtu)
{
    if (conn_id >= 0 && conn_id < MAX_LE_CLIENT_CONN) {
        Connection *connection = &(conns[conn_id]);
        int att_len = mtu - 2 - 1; // att_len = mtu - opcode - handle;
        connection->att_mtu_max = att_len;
    }
}

int btcli_gatts_attributedata_get_max_mtu(int conn_id)
{
    if (conn_id >= 0 && conn_id < MAX_LE_CLIENT_CONN) {
        Connection *connection = &(conns[conn_id]);
        return connection->att_mtu_max;
    }
    return 0;
}
...
enum cmd_status btcli_gattc_unsubscribe(char *cmd)
{
    ...
    if (err) {
        CMD_ERR("btmg_gattc_unsubscribe failed (err %d)\n", err);
