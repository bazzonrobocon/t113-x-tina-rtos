...
// fix unused-function
__attribute__((__unused__)) static uint8_t char2uint8(char* trs)
{
	uint8_t ret = 0;
	uint8_t tmp_ret[2] = {0};
	int i = 0;
...
// fix unused-function
__attribute__((__unused__)) static int isdigitstr(char *str)
{
...
static int cmd_handle_s(char *p)
{
	...
	if(!strncmp(p, "p2p", 3)) {
		...
		if (ret == 0) {
			...
			WMG_INFO("==Wi-Fi scan successful, total %d ap(scan_max_num: %d, ssid_max_len: 32) time %4f ms==\n", bss_num, RES_LEN, USE_TIME(begin, end));
			return 0;
		} else {
...
static int cmd_handle_c(char *p)
{
	...
	int ret = -1;
	int i, get_scan_results_flag = 0, get_scan_results_num = 0;
	uint32_t bss_num = 0;
	wifi_scan_result_t scan_res[RES_LEN] = {0};
	char scan_res_char[20] = {0};
	...
	if(psk_p == NULL) {
		...
		if(sec_p != NULL) {
			...
			if(get_scan_results_flag) {
				...
				WMG_ERROR("==Wi-Fi can't scan ssid:%s==\n", ssid_buf);
				return -1;
			}
		} else {
...
static int cmd_handle_r(char *p)
{
	...
	if(!strncmp(p, "all", 3)) {
		...
		if(strlen(p) > SSID_MAX_LEN){
			...
			WMG_INFO("remove networks(%s) %s\n", p, (ret == WMG_STATUS_SUCCESS)?"success":"fail");
		}
	}
	return ret;
}
...
#ifdef OS_NET_LINUX_OS
	strcat(send_expand_cmd, "linux: ");
#endif
#ifdef OS_NET_XRLINK_OS
	strcat(send_expand_cmd, "xrlink: ");
#endif
#ifdef OS_NET_FREERTOS_OS
	strcat(send_expand_cmd, "freertos: ");
#endif
	strcat(send_expand_cmd, "gmac: wlan0");

	ret = wifi_send_expand_cmd(send_expand_cmd, mac_addr);
	if (ret == WMG_STATUS_SUCCESS) {
...
static int cmd_handle_m(char *p)
{
	...
	strcat(send_expand_cmd, "freertos: ");
#endif
	strcat(send_expand_cmd, "smac: wlan0: ");
	strcat(send_expand_cmd, p);

	ret = wifi_send_expand_cmd(send_expand_cmd, NULL);
	if (ret == WMG_STATUS_SUCCESS) {
		WMG_INFO("set macaddr: %s %s\n", p, (ret == WMG_STATUS_SUCCESS)?"success":"faile");
	}
	return ret;
}
