...
{
	int ret = 0;
	unsigned int *standby_bin_start=_standby_bin_start;
	...
	if (hpsram.inited) {
#if defined(CONFIG_PROJECT_XR875)
...
#if CONFIG_ARCH_DSP_START_ADDRESS
		if ((CONFIG_ARCH_DSP_START_ADDRESS & 0x0f000000) == 0x0c000000) {
			pm_warn("hspsram will not be suspended\n");
			head->hpsram_inited = 0;
		}
#else
	pm_warn("fail to check the DSP running address, DSP may run in PSRAM\n");
...
static int pm_m33_begin(suspend_mode_t mode)
{
	...
	int id = 0;
	int awake_subsys = 0;
	__attribute__((__unused__)) int32_t subcnt;
#if defined(CONFIG_PM_SUBSYS_DSP_SUPPORT) || defined(CONFIG_PM_SUBSYS_RISCV_SUPPORT)
	int num = 0;
#endif
...
static int pm_m33_again_late(suspend_mode_t mode)
{
	int ret = 0;
	int subsys_assert = 0;
	__attribute__((__unused__)) int subsys_assert_save = 0;
