#ifndef __TT_TEST_H__
#define __TT_TEST_H__
...
static int cmd_ble_test(int argc, char **argv)
{
	int err;
	...
	if (err == -EALREADY) {
		...
		err = bt_gatt_register_svc_test();
		if (err) {
			printf("[gatt add svc] err=%d\n", err);
		}
...
static int cmd_ble_scan(int flag)
{
	...
	if (flag == 0) {
		...
		printf("error param please check\n");
	}
	return -1;
}
