						     ...
						     struct rpbuf_buffer *buffer)
{
	...
	buffer->va = addr;
	ret = mem_va_to_pa((unsigned long)addr, &lpa, NULL);
	if (ret)
		return ret;
	buffer->pa = (uintptr_t)lpa;
				      ...
				      uintptr_t pa, uint64_t da, int len)
{
	...
	if (mem_pa_to_va((unsigned long)pa, &lva, NULL)) {
		rpbuf_err("invalid pa: 0x%lx\r\n", (long)pa);
		return NULL;
	}

	rpbuf_info("reamp pa:0x%lx -> va:0x%lx\r\n", (long)pa, (long)lva);
								...
								struct rpbuf_buffer *buffer)
{
	...
	if (!(buffer->state & RPBUF_FLAGS_GETNOTIFY))
		hal_wait_event(buffer->wait, (buffer->state & (RPBUF_FLAGS_GETNOTIFY | RPBUF_FLAGS_DESTROYED)));
							  ...
							  void *content)
{
	...
	if (buffer) {
		...
		rpbuf_info("wakeup buffer %s, state %u\n", buffer->name, buffer->state);
		hal_wake_up(&buffer->wait);
	}
...
static int rpbuf_free_buffer_internal(struct rpbuf_buffer *buffer, int do_notify)
{
	...
	if (buffer->state & RPBUF_FLAGS_WORKING) {
		...
		rpbuf_dbg("buffer \"%s\"(id:%d): Wait for cb complete\n", buffer->name, buffer->id);
		hal_wait_event(buffer->wait, !(buffer->state & (RPBUF_FLAGS_WORKING)));
		rpbuf_mutex_lock(link->controller_lock);
	}
