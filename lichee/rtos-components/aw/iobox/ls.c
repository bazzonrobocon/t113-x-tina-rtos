    ...
    for (index = 0; index < argc; index++) {
		...
		if (!S_ISDIR(s.st_mode)) {
            ...
            while ((entry = readdir(pdir))) {
                 ...
                 */
                snprintf(fpath, sizeof(fpath), "/data/%s", entry->d_name);
                if (strncmp(dir, fpath, len))
                    continue;
#else
		assert(strlen(dir) < 64);
		assert(strlen(entry->d_name) < 64);
                if (dir[len] == '/' || len == 1)
                    snprintf(fpath, sizeof(fpath), "%.*s%.*s", 64, dir, 63, entry->d_name);
                else
                    snprintf(fpath, sizeof(fpath), "%.*s/%.*s", 63, dir, 63, entry->d_name);
#endif
                if (stat(fpath, &s)) {
