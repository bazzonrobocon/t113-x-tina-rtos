...
static ssize_t devfs_vfs_write(int fd, const void *data, size_t size)
{
    ...
    return wlen;
err:
    pr_err("write to fd %d with size %zu failed\n", fd, size);
    return ret;
}
...
static ssize_t devfs_vfs_read(int fd, void *data, size_t size)
{
    ...
    return rlen;
err:
    pr_err("read from fd %d with size %zu failed\n", fd, size);
    return ret;
}
...
struct devfs_node* devfs_get_node(const char *name)
{
    struct devfs_node *n;

    for (n = devfs->node; n; n = n->next) {
