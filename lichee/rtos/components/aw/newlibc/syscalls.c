...
extern int amp_log_put_buf(char *buf, int len);
#endif
...
_ssize_t _write_r(struct _reent *ptr, int fd, const void *buf, size_t nbytes)
{
    if (fd < 3) {
...
#ifdef CONFIG_AMP_TRACE_SUPPORT
        amp_log_put_buf((char *)buf, nbytes);
#endif

#if defined(CONFIG_COMPONENTS_MULTI_CONSOLE) || defined(CONFIG_MULTI_CONSOLE)
        cli_console_write(get_clitask_console(), buf, nbytes);
#else
        ...
        for (i = 0; i < nbytes; i ++)
        {
            if (*(b + i) == '\n')
            {
                hal_uart_send(CONFIG_CLI_UART_PORT, (unsigned char *)"\r", 1);
            }
            hal_uart_send(CONFIG_CLI_UART_PORT, (unsigned char *)(b + i), 1);
        }
#endif
#endif
        return (nbytes);
