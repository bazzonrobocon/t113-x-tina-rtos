...
int blkpart_erase(struct part *part, uint32_t offset, uint32_t size)
{
    ...
    if (offset + size > part->bytes)
        pr_warn("erase %s(%s) over limit: offset %u + size %u over %"PRIu64"\n",
        ...
        uint32_t size, const void *data, bool erase_before_write)
{
    ...
    if (offset >= part->bytes) {
        pr_warn("write offset %u over part size %"PRIu64"\n", offset, part->bytes);
        return 0;
    }

    if (offset + size > part->bytes)
        pr_warn("write %s(%s) over limit: offset %u + size %u over %"PRIu64"\n",
                part->name, part->devname, offset, size, part->bytes);
        ...
        void *data)
{
    ...
    if (offset >= part->bytes) {
        pr_warn("read offset %u over part size %"PRIu64"\n", offset, part->bytes);
        return 0;
    }

    if (offset + size > part->bytes)
        pr_warn("read %s(%s) over limit: offset %u + size %u over %"PRIu64"\n",
                part->name, part->devname, offset, size, part->bytes);
...
static int part_info_main(int argc, char **argv)
{
    ...
    for (i = 0; i < PARTINDEX_THE_LAST; i++) {
        ...
        pr_info("%-16s(%-8s):\tbytes 0x%-16"PRIx64"\toff 0x%-16"PRIx64"\tflag:0x%-8x\n", part->name, part->devname,
