...
static int lfs_vfs_ftruncate(void *_ctx, int fd, off_t length)
{
    ssize_t ret;
    struct lfs_ctx *ctx = _ctx;
    lfs_file_t *file = lfs_get_entry(ctx, fd);
    ...
    lfs_lock(ctx);
    ret = lfs_file_truncate(ctx->lfs, file, length);
    lfs_unlock(ctx);
...
static int lfs_vfs_truncate(void *_ctx, const char *path, off_t length)
{
    ssize_t ret;
    int fd = lfs_vfs_open(_ctx, path, O_RDWR, 0);
    if(fd == -1)
        return fd;
    ret = lfs_vfs_ftruncate(_ctx, fd, length);
    lfs_vfs_close(_ctx, fd);
...
int lfs_register_vfs(struct lfs_ctx *ctx)
{
    const vfs_t vfs = {
        ...
        .truncate_p = &lfs_vfs_truncate,
        .ftruncate_p = &lfs_vfs_ftruncate,
    };
