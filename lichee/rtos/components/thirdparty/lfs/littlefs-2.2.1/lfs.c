...
static int lfs_alloc(lfs_t *lfs, lfs_block_t *block) {
    while (true) {
        ...
        if (lfs->free.ack == 0) {
            LFS_ERROR("No more free space %u",
...
static lfs_stag_t lfs_dir_fetchmatch(lfs_t *lfs,
    ...
    LFS_ERROR("Corrupted dir pair at {0x%x, 0x%x}",
    ...
    if (lfs->cfg->block_cycles > 0 &&
        ...
        if (lfs_pair_cmp(dir->pair, (const lfs_block_t[2]){0, 1}) == 0) {
            ...
            if ((lfs_size_t)res < lfs->cfg->block_count/2) {
                LFS_DEBUG("Expanding superblock at rev %u", dir->rev);
...
relocate:
        ...
        if (!tired) {
            LFS_DEBUG("Bad block at 0x%x", dir->pair[1]);
        }

        // can't relocate superblock, filesystem is now frozen
        if (lfs_pair_cmp(dir->pair, (const lfs_block_t[2]){0, 1}) == 0) {
            LFS_WARN("Superblock 0x%x has become unwritable",
                    dir->pair[1]);
            return LFS_ERR_NOSPC;
        }
    ...
    if (relocated) {
        ...
        LFS_DEBUG("Relocating {0x%x, 0x%x} "
                    "-> {0x%x, 0x%x}",
...
relocate:
                LFS_DEBUG("Bad block at 0x%x", file->block);
...
lfs_ssize_t lfs_file_read(lfs_t *lfs, lfs_file_t *file,
        void *buffer, lfs_size_t size) {
    LFS_TRACE("lfs_file_read(%p, %p, %p, %u)",
...
lfs_ssize_t lfs_file_write(lfs_t *lfs, lfs_file_t *file,
        const void *buffer, lfs_size_t size) {
    LFS_TRACE("lfs_file_write(%p, %p, %p, %u)",
            (void*)lfs, (void*)file, buffer, size);
    LFS_ASSERT(file->flags & LFS_F_OPENED);
...
lfs_ssize_t lfs_getattr(lfs_t *lfs, const char *path,
    ...
    LFS_TRACE("lfs_getattr(%p, \"%s\", %u, %p, %u)",
            (void*)lfs, path, type, buffer, size);
    lfs_mdir_t cwd;
    lfs_stag_t tag = lfs_dir_find(lfs, &cwd, &path, NULL);
    if (tag < 0) {
        LFS_TRACE("lfs_getattr -> %d", tag);
        return tag;
    }
...
int lfs_setattr(lfs_t *lfs, const char *path,
        uint8_t type, const void *buffer, lfs_size_t size) {
    LFS_TRACE("lfs_setattr(%p, \"%s\", %u, %p, %u)",
            (void*)lfs, path, type, buffer, size);
...
int lfs_mount(lfs_t *lfs, const struct lfs_config *cfg) {
        ...
        if (tag && !lfs_tag_isdelete(tag)) {
            ...
            if (superblock.name_max) {
                if (superblock.name_max > lfs->name_max) {
                    LFS_ERROR("Unsupported name_max (%u > %u)",
            ...
            if (superblock.file_max) {
                if (superblock.file_max > lfs->file_max) {
                    LFS_ERROR("Unsupported file_max (%u > %u)",
            ...
            if (superblock.attr_max) {
                if (superblock.attr_max > lfs->attr_max) {
                    LFS_ERROR("Unsupported attr_max (%u > %u)",
    ...
    if (!lfs_gstate_iszero(&lfs->gstate)) {
        LFS_DEBUG("Found pending gstate 0x%08x%08x%08x",
                lfs->gstate.tag,
                lfs->gstate.pair[0],
                lfs->gstate.pair[1]);
    }
...
static int lfs_fs_demove(lfs_t *lfs) {
    ...
    // Fix bad moves
    LFS_DEBUG("Fixing move {0x%x, 0x%x} 0x%x",
            lfs->gdisk.pair[0],
            lfs->gdisk.pair[1],
...
static int lfs_fs_deorphan(lfs_t *lfs) {
    ...
    while (!lfs_pair_isnull(pdir.tail)) {
        ...
        if (!pdir.split) {
            ...
            if (tag == LFS_ERR_NOENT) {
                // we are an orphan
                LFS_DEBUG("Fixing orphan {0x%x, 0x%x}",
            ...
            if (!lfs_pair_sync(pair, pdir.tail)) {
                // we have desynced
                LFS_DEBUG("Fixing half-orphan {0x%x, 0x%x} "
                            "-> {0x%x, 0x%x}",
...
static int lfs1_mount(lfs_t *lfs, struct lfs1 *lfs1,
    ...
    int err = 0;
    {
        ...
        if (err || memcmp(superblock.d.magic, "littlefs", 8) != 0) {
            LFS_ERROR("Invalid superblock at {0x%x, 0x%x}",
                    0, 1);
            err = LFS_ERR_CORRUPT;
            goto cleanup;
        }
