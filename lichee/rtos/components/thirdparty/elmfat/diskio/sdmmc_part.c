...
extern int sunxi_sdmmc_init(struct devfs_node* dev);
extern int elmfat_mount(const char *dev, const char *base_path);
extern int elmfat_unmount(const char *base_path, struct devfs_node *node);
...
extern int _sunxi_sdmmc_write(unsigned int pos, char *buffer, unsigned int size);
extern int _sunxi_sdmmc_read(unsigned int pos, char *buffer, unsigned int size);
...
static int register_emmc_blk_device(struct devfs_node *dev)
{
    ...
    card = mmc_card_open((uint8_t )((long )dev->private));
    if (!card)
    {
        ret = -1;
        goto err;
...
#define EMMC_GPT_ADDR (0)
    ...
    foreach_gpt_part(gpt_buf, gpt_part)
    {
        ...
        snprintf(part->name, MAX_BLKNAME_LEN, "%.*s", MAX_BLKNAME_LEN - 1, gpt_part->name);
    ...
    if (card) {
        mmc_card_close((uint8_t )((long )dev->private));
    }
    hal_free(gpt_buf);
    return 0;
...
void unmount_sdmmc_filesystem(int card_id)
{
    ...
    card = mmc_card_open((uint8_t )card_id);
    if (!card) {
        return;
    }
