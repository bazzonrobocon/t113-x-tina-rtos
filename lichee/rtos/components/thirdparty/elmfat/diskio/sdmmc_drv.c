...
int sunxi_sdmmc_init(struct devfs_node * dev)
{
    ...
    sdc_param.dma_use = 1;
    dev->private = (void *)((unsigned long )host_id);
...
ssize_t sunxi_sdmmc_read(struct devfs_node * dev, uint32_t pos, uint32_t size, void *buffer)
{
    ...
    struct mmc_card *card = mmc_card_open((uint8_t )((long )dev->private));
    if (card == NULL)
    {
...
int _sunxi_sdmmc_write(unsigned int pos, char *buffer, unsigned int size)
{
    size_t ret;
    struct mmc_card *card = mmc_card_open(0);
    ret = mmc_block_write(card, (uint8_t *)buffer, pos / 512, 1);
    mmc_card_close(0);
    return ret;
}

int _sunxi_sdmmc_read(unsigned int pos, char *buffer, unsigned int size)
{
    size_t ret;
    struct mmc_card *card = mmc_card_open(0);
