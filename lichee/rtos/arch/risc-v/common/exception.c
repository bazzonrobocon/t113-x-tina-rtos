...
#else
    panic_printf("\ngprs:\n");
    panic_printf(" x0:0x%08x   ra:0x%08lx   sp:0x%08lx   gp:0x%08lx\n", 0        , regs->x1 , regs->x2 , regs->x3 );
    panic_printf(" tp:0x%08lx   t0:0x%08lx   t1:0x%08lx   t2:0x%08lx\n", regs->x4 , regs->x5 , regs->x6 , regs->x7 );
    panic_printf(" s0:0x%08lx   s1:0x%08lx   a0:0x%08lx   a1:0x%08lx\n", regs->x8 , regs->x9 , regs->x10, regs->x11);
    panic_printf(" a2:0x%08lx   a3:0x%08lx   a4:0x%08lx   a5:0x%08lx\n", regs->x12, regs->x13, regs->x14, regs->x15);
    panic_printf(" a6:0x%08lx   a7:0x%08lx   s2:0x%08lx   s3:0x%08lx\n", regs->x16, regs->x17, regs->x18, regs->x19);
    panic_printf(" s5:0x%08lx   s5:0x%08lx   s6:0x%08lx   s7:0x%08lx\n", regs->x20, regs->x21, regs->x22, regs->x23);
    panic_printf(" s8:0x%08lx   s9:0x%08lx  s10:0x%08lx  s11:0x%08lx\n", regs->x24, regs->x25, regs->x26, regs->x27);
    panic_printf(" t3:0x%08lx   t4:0x%08lx   t5:0x%08lx   t6:0x%08lx\n", regs->x28, regs->x29, regs->x30, regs->x31);
...
#ifdef CONFIG_ARCH_RISCV_PMP
    extern int cmd_pmpdump(int argc, const char **argv);
    if ((cause == EXC_LOAD_ACCESS)
        || (cause == EXC_STORE_ACCESS)
        || (cause == EXC_INST_ACCESS))
    {
        printf("\n--------PMP config and check result--------\n");
        cmd_pmpdump(0, NULL);
        printf("\n");

        if (cause == EXC_LOAD_ACCESS)
            pmp_check_addr_permission(mtval, PMP_R);
        if (cause == EXC_STORE_ACCESS)
            pmp_check_addr_permission(mtval, PMP_W);
        if (cause == EXC_INST_ACCESS)
            pmp_check_addr_permission(mtval, PMP_X);
        printf("-------------------------------------------\n\n");

#ifdef CONFIG_PMP_DEBUG_CONFIG_SEQUENCE
        printf("--------PMP config record--------\n");
