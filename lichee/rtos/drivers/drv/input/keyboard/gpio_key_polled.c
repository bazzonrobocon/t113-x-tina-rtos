...
int gpio_key_polled_init(void)
{
	...
	task_ret = xTaskCreate(gpio_key_polled_poll, "gpio_key_poll", 256, key_drvdata, configTIMER_TASK_PRIORITY - 1, NULL);
	if (task_ret != pdPASS) {
		printf("Error creating gpio_key_polled_poll task, status was %ld\n", (long)task_ret);
		return -1;
	}
