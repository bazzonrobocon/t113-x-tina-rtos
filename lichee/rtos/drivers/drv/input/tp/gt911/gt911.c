...
static int gt911_get_product_id(struct gt911_drv_data *dev)
{
	...
	if (gt911_i2c_read(dev, (char *)reg, 2, (char *)data, len) != 0) {
...
static int gt911_get_info(struct gt911_drv_data *dev)
{
	...
	if (gt911_i2c_read(dev, (char *)reg, 2, (char *)out_info, out_len) != 0) {
...
static int gt911_config_freq(struct gt911_drv_data *dev)
{
	...
	if (gt911_i2c_write(dev, (char *)buf, 3) != 0) {
...
static int gt911_init_config(struct gt911_drv_data *data)
{
	...
	config[16] &= 0x0F; //rate
	if (gt911_i2c_write(data, (char *)config, sizeof(GT911_CFG_TBL) + GT911_ADDR_LEN) != 0) {
	...
	gt911_i2c_write(data, (char *)buf, 4);
	free(config);
	return 0;
}
*/
...
static int gt911_update_data(struct gt911_drv_data *parm)
{
	...
	ret = gt911_i2c_read(data, (char *)write_buf, 2, (char *)&point_status, 1);
	...
	ret = gt911_i2c_read(data, (char *)write_buf, 2, (char *)read_buf, read_num * GT911_POINT_INFO_NUM);
...
exit_:
	...
	write_buf[1] = (uint8_t)(GT911_READ_STATUS & 0xFF);
	write_buf[2] = 0x00;
	gt911_i2c_write(data, (char *)write_buf, 3);
...
// When using the lvgl, this cannot be enabled
	...
	ret = hal_gpio_to_irq(config->int_gpio, (uint32_t *)&data->irq_num);
	if (ret < 0) {
...
int gt911_init(void)
{
	...
	task_ret = xTaskCreate(touch_event_handler, "touch_event_handle_task", 1024, gt911_data, 0, NULL);
	if (task_ret != pdPASS) {
		GT911_ERR("input dev alloc err\n");
		goto err_free_sem;
