...
static HAL_SPIN_LOCK_INIT(os_timer_restore_lock);
                               ...
                               unsigned char flag)
{
    ...
    if (priv == NULL)
    {
        hal_log_warn("Out of memory\n");
        return NULL;
    }
    memset(priv, 0, sizeof(os_timer_data_t));
    priv->callback = timeout;
    priv->arg = parameter;
    timer = xTimerCreate(name, tick, flag & OSAL_TIMER_FLAG_PERIODIC ? pdTRUE : pdFALSE, \
                    priv, os_timer_common_callback);
    if (timer) {
        priv->xTimer = timer;
        priv->inlist = 1;
        priv->reseting = 0;
...
hal_status_t osal_timer_start_all(void)
{
    ...
    ccpsr = hal_spin_lock_irqsave(&os_timer_restore_lock);
    list_for_each_safe(list_node, list_save, list) {
        priv = container_of(list_node, os_timer_data_t, node);
        if (priv && priv->xTimer) {
    ...
    hal_spin_unlock_irqrestore(&os_timer_restore_lock, ccpsr);
