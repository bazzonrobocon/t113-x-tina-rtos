...
//jpeg
	...
	hal_waitqueue_t csi_waitqueue;
	hal_waitqueue_t jpeg_waitqueue;
...
struct csi_jpeg_mem *hal_csi_dqbuf(unsigned int timeout_msec)
{
	...
	ret = hal_waitqueue_wait(&csi_jpeg_priv->csi_waitqueue, list_empty(&csi_jpeg_priv->csi_done) == 0, timeout_msec);
	if (ret) {
...
struct csi_jpeg_mem *hal_jpeg_dqbuf(unsigned int timeout_msec)
{
	...
	ret = hal_waitqueue_wait(&csi_jpeg_priv->jpeg_waitqueue, list_empty(&csi_jpeg_priv->jpeg_done) == 0, timeout_msec);
	if (ret) {
...
static hal_irqreturn_t csi_jpeg_isr(void *data)//CSI_JPEG_IRQHandler pay attention
{
	...
	if (csi_int_status & CSI_INT_STA_FRM_END_PD) {
		...
		if (csi_jpeg_priv->output_fmt.line_mode == OFFLINE_MODE) {
			...
			hal_waitqueue_wakeup(&csi_jpeg_priv->csi_waitqueue, NULL);
		}
	}
...
jpeg_isr:
	...
	if (jpeg_int_status & JPEG_ENC_FINISH) {
		...
		hal_waitqueue_wakeup(&csi_jpeg_priv->jpeg_waitqueue, NULL);
	}
...
HAL_Status hal_csi_jpeg_probe(void)
{
	...
	hal_waitqueue_init(&csi_jpeg_priv->csi_waitqueue);
//jpeg
	hal_waitqueue_init(&csi_jpeg_priv->jpeg_waitqueue);
...
HAL_Status hal_csi_jpeg_remove(void)
{
	...
	hal_waitqueue_deinit(&csi_jpeg_priv->csi_waitqueue);
	hal_waitqueue_deinit(&csi_jpeg_priv->jpeg_waitqueue);
