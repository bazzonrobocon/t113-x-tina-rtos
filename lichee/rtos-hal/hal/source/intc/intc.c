...
s32 intc_enable_interrupt(u32 intno)
{
	...
	u32 group_num = ((major - SUNXI_RINTC_IRQ_GROUP_START) *
			SUNXI_RINTC_IRQ_GROUP_NUM + minor);
	...
	if (major >= SUNXI_RINTC_IRQ_GROUP_START &&
		...
		if (group_num <= 31)
			stby_intc_regs->group_config0 |= (1 << group_num);
		else if (group_num > 31 && group_num <= 63)
			stby_intc_regs->group_config1 |= (1 << (group_num - 32));
		else
			stby_intc_regs->group_config2 |= (1 << (group_num - 64));
	}
