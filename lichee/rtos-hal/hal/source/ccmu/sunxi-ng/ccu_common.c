...
int ccu_common_init(unsigned long reg, const struct sunxi_ccu_desc *desc)
{
    ...
    int clk_parents_num = 0;

    struct ccu_slab *slab_core;
    struct ccu_slab *slab_clk;
    struct ccu_slab *slab_parents;
    ...
    slab_core = ccu_slab_init(sizeof(struct clk_core), desc->hw_clks->num, "clk_core");
    if (!slab_core) {
	    ret = -1;
	    return ret;
    }

    slab_clk = ccu_slab_init(sizeof(struct clk), desc->hw_clks->num, "clk");
    if (!slab_clk) {
	    ret = -1;
	    goto err_slab_err1;
    }
	...
	init = hw->init;
	clk_parents_num += init->num_parents;
    }

    slab_parents = ccu_slab_init(sizeof(struct clk_parent_map), clk_parents_num, "clk_parents");
    if (!slab_parents) {
	    ret = -1;
	    goto err_slab_err2;
    }
    ...
    for (i = 0; i < desc->hw_clks->num; i++)
    {
        ...
        ret = clk_hw_register_with_slab(hw, slab_core, slab_clk, slab_parents);
        if (ret)
        {
...
err_clk_unreg:
    while (i-- >= 0)
    {
        ...
        clk_hw_unregister_with_slab(hw);
    }
    ccu_slab_deinit(slab_parents);
err_slab_err2:
    ccu_slab_deinit(slab_clk);
err_slab_err1:
    ccu_slab_deinit(slab_core);
    return ret;
}
