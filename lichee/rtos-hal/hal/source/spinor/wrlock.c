...
{
    if (nor->factory->init_lock && !(nor->info->flag & SUPPORT_WITHOUT_PROTECT))
...
{
    if (nor->factory->deinit_lock && !(nor->info->flag & SUPPORT_WITHOUT_PROTECT))
...
bool nor_wr_islock(struct nor_flash *nor, unsigned int addr, unsigned int len)
{
    ...
    if (nor->factory->islock && !(nor->info->flag & SUPPORT_WITHOUT_PROTECT)) {
        while (len) {
...
int nor_wr_lock(struct nor_flash *nor, unsigned int addr, unsigned int len)
{
    ...
    if (nor->factory->lock && !(nor->info->flag & SUPPORT_WITHOUT_PROTECT)) {
        while (len) {
            /* the first and the last block (64K) is lock on sector (4K) unit */
            if (addr >= nor->total_size - SZ_64K || addr < SZ_64K)
                align_size = SZ_4K;
...
int nor_wr_unlock(struct nor_flash *nor, unsigned int addr, unsigned int len)
{
    ...
    if (nor->factory->unlock && !(nor->info->flag & SUPPORT_WITHOUT_PROTECT)) {
        while (len) {
            /* the first and the last block (64K) is lock on sector (4K) unit */
            if (addr >= nor->total_size - SZ_64K || addr < SZ_64K)
                align_size = SZ_4K;
