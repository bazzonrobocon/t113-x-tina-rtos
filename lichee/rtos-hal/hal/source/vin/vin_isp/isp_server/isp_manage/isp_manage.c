...
void __isp_awb_run(struct isp_lib_context *isp_gen)
{
	...
	if (param->isp_test_settings.nrp_en && mod_cfg->nrp_cfg.gamma_en &&
		...
		if (isp_gen->ai_isp_en == 1) {
			isp_gen->module_cfg.wb_gain_cfg.wb_gain.r_gain = (HW_U16)(pow((double)isp_awb_cxt->awb_result.wb_gain_output.r_gain / 256.0, 1/2.2) * 256.0);
			isp_gen->module_cfg.wb_gain_cfg.wb_gain.gr_gain = (HW_U16)(pow((double)isp_awb_cxt->awb_result.wb_gain_output.gr_gain / 256.0, 1/2.2) * 256.0);
			isp_gen->module_cfg.wb_gain_cfg.wb_gain.gb_gain = (HW_U16)(pow((double)isp_awb_cxt->awb_result.wb_gain_output.gb_gain / 256.0, 1/2.2) * 256.0);
			isp_gen->module_cfg.wb_gain_cfg.wb_gain.b_gain = (HW_U16)(pow((double)isp_awb_cxt->awb_result.wb_gain_output.b_gain / 256.0, 1/2.2) * 256.0);
		} else if (isp_gen->ai_isp_en == 2) {
			isp_gen->module_cfg.wb_gain_cfg.wb_gain.r_gain = (HW_U16)(pow((double)isp_awb_cxt->awb_result.wb_gain_output.r_gain / 256.0, 1/1.69) * 256.0);
			isp_gen->module_cfg.wb_gain_cfg.wb_gain.gr_gain = (HW_U16)(pow((double)isp_awb_cxt->awb_result.wb_gain_output.gr_gain / 256.0, 1/1.69) * 256.0);
			isp_gen->module_cfg.wb_gain_cfg.wb_gain.gb_gain = (HW_U16)(pow((double)isp_awb_cxt->awb_result.wb_gain_output.gb_gain / 256.0, 1/1.69) * 256.0);
			isp_gen->module_cfg.wb_gain_cfg.wb_gain.b_gain = (HW_U16)(pow((double)isp_awb_cxt->awb_result.wb_gain_output.b_gain / 256.0, 1/1.69) * 256.0);
...
static void __isp_ctx_cfg_mod(struct isp_lib_context *isp_gen)
{
	...
	if (isp_gen->sensor_info.fps_fixed > 60) {
		isp_gen->awb_settings.wb_stat_combine_mode = WB_STAT_ROW_COL_COMBINE;
	} else {
...
int __isp_dump_reg(struct isp_lib_context *isp_gen)
{
	...
	__maybe_unused int i, *reg;
#else
	ATTRIBUTE_UNUSED int i, *reg;
#endif
