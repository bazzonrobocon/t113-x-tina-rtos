...
int isp_get_debug_info(struct hw_isp_device *hw_isp, int *rpdata, int rpdata_len)
{
		...
		size_t len, uint32_t src, void *priv)
{
	...
	case VIN_REQUEST_ATTR_IOCTL:
		...
		break;
	case VIN_SYNC_ISP_INFO:
		rpdata[0] = ISP_SYNC_INFO_TO_VIN;
		isp_get_debug_info(hw_isp, &rpdata[1], (RPMEG_BUF_LEN - 1));
		isp_rpmsg_ept_send(hw_isp->ept, rpdata, RPMEG_BUF_LEN * 4);
		break;
