...
//param + isp version + special ctrl + log_cfg + param offset
#define ISP_RPBUF_TUNNING_LEN (sizeof(struct isp_param_config) + sizeof(HW_U16) + 2*sizeof(HW_U8) + \
						+ sizeof(struct isp_log_cfg) + (3*sizeof(isp_dynamic_triger_t) / sizeof(enum isp_triger_type)))
...
#define RPMSG_SERVICE_NAME "sunxi,isp_parameter"
...
struct hw_isp_device {
	...
	hal_mutex_t buffer_send_lock;
	hal_workqueue *isp_rpbuf_run_task;
	hal_work isp_rpbuf_server_work;
	struct rpmsg_endpoint *isp_para_ept;
	int isp_rpbuf_tunning_len;

	void (*isp_tdm_buffer_process)(unsigned int *status);
};
