...
int isp_params_parse(struct hw_isp_device *isp, struct isp_param_config *params, HW_U8 ir, int sync_mode)
{
	...
	if (ctx->sensor_info.fps_fixed >= ISP_FAST_CONVERGENCE_FPS) {
		...
		(isp->id != 0 && sensor_isp_cfg->sign == 0xBB66BB66)) {
			return parser_ini_info(params, ctx->isp_cfg_name, ctx->sensor_info.name,
...
int isp_set_ini_tunning_info(struct hw_isp_device *hw_isp, void *src)
{
	struct isp_lib_context *isp_gen = isp_dev_get_ctx(hw_isp);
	struct isp_tuning *tuning = isp_dev_get_tuning(hw_isp);
	struct isp_lib_context *ctx = isp_dev_get_ctx(hw_isp);
	char *char_ptr = NULL;
	short *short_ptr = NULL;
	int *int_ptr = NULL;
	enum isp_triger_type *triger_dst = NULL;
	int i, len;
	...
	//isp_test_settings
	char_ptr = (char *)src;
	memcpy(&tuning->params.isp_test_settings, char_ptr, sizeof(struct isp_test_param));
	char_ptr += sizeof(struct isp_test_param);
	//isp_3a_settings
	memcpy(&tuning->params.isp_3a_settings, char_ptr, sizeof(struct isp_3a_param));
	char_ptr += sizeof(struct isp_3a_param);
	//isp_tunning_settings
	memcpy(&tuning->params.isp_tunning_settings, char_ptr, sizeof(struct isp_tunning_param));
	char_ptr += sizeof(struct isp_tunning_param);
	//isp_iso_settings
	int_ptr = (int *)char_ptr;
	triger_dst = (enum isp_triger_type *)&tuning->params.isp_iso_settings.triger;
	len = sizeof(isp_dynamic_triger_t) / sizeof(enum isp_triger_type);
	for (i = 0; i < len; i++) {
		*triger_dst = *int_ptr;
		int_ptr++;
		triger_dst++;
	...
	memcpy(&tuning->params.isp_iso_settings.isp_lum_mapping_point[0], char_ptr, 14 * sizeof(HW_S32));
	char_ptr += 14 * sizeof(HW_S32);
	memcpy(&tuning->params.isp_iso_settings.isp_gain_mapping_point[0], char_ptr, 14 * sizeof(HW_S32));
	char_ptr += 14 * sizeof(HW_S32);
	memcpy(&tuning->params.isp_iso_settings.isp_dynamic_cfg[0], char_ptr, 14 * sizeof(struct isp_dynamic_config));
	char_ptr += 14 * sizeof(struct isp_dynamic_config);
	//skip isp version
	short_ptr = (short *)char_ptr;
	short_ptr++;
	//special_ctrl
	char_ptr = (char *)short_ptr;
	if (*char_ptr != isp_gen->isp_ir_flag) {
		isp_gen->isp_ir_flag = *char_ptr;
		if (isp_gen->isp_ir_flag) {
	...
	char_ptr++;
	if (*char_ptr != isp_gen->sensor_info.color_space) {
		isp_gen->sensor_info.color_space = *char_ptr;
		ctx->isp_3a_change_flags |= ISP_SET_EFFECT;
	}
	char_ptr++;
...
int isp_get_ini_tunning_info(struct hw_isp_device *hw_isp, void *dst)
{
	struct isp_lib_context *isp_gen = isp_dev_get_ctx(hw_isp);
	struct isp_tuning *tuning = isp_dev_get_tuning(hw_isp);
	char *char_ptr = NULL;
	short *short_ptr = NULL;
	int *int_ptr = NULL;
	enum isp_triger_type *triger_src = NULL;
	struct isp_log_cfg *log_cfg = NULL;
	int i, len;
	...
	//isp_test_settings
	char_ptr = (char *)dst;
	memcpy(char_ptr, &tuning->params.isp_test_settings, sizeof(struct isp_test_param));
	char_ptr += sizeof(struct isp_test_param);
	//isp_3a_settings
	memcpy(char_ptr, &tuning->params.isp_3a_settings, sizeof(struct isp_3a_param));
	char_ptr += sizeof(struct isp_3a_param);
	//isp_tunning_settings
	memcpy(char_ptr, &tuning->params.isp_tunning_settings, sizeof(struct isp_tunning_param));
	char_ptr += sizeof(struct isp_tunning_param);
	//isp_iso_settings
	int_ptr = (int *)char_ptr;
	triger_src = (enum isp_triger_type *)&tuning->params.isp_iso_settings.triger;
	len = sizeof(isp_dynamic_triger_t) / sizeof(enum isp_triger_type);
	for (i = 0; i < len; i++) {
		*int_ptr = *triger_src;
		int_ptr++;
	...
	char_ptr = (char *)int_ptr;
	memcpy(char_ptr, &tuning->params.isp_iso_settings.isp_lum_mapping_point[0], 14 * sizeof(HW_S32));
	char_ptr += 14 * sizeof(HW_S32);
	memcpy(char_ptr, &tuning->params.isp_iso_settings.isp_gain_mapping_point[0], 14 * sizeof(HW_S32));
	char_ptr += 14 * sizeof(HW_S32);
	memcpy(char_ptr, &tuning->params.isp_iso_settings.isp_dynamic_cfg[0], 14 * sizeof(struct isp_dynamic_config));
	char_ptr += 14 * sizeof(struct isp_dynamic_config);
	//isp version
	short_ptr = (short *)char_ptr;
	*short_ptr = ISP_VERSION;
	short_ptr++;
	//special_ctrl
	char_ptr = (char *)short_ptr;
	*char_ptr = isp_gen->isp_ir_flag;
	char_ptr++;
	*char_ptr = isp_gen->sensor_info.color_space;
	char_ptr++;
	//log_cfg
	log_cfg = hal_malloc(sizeof(struct isp_log_cfg));//avoid stack over 4kb
	if (!log_cfg) {
		ISP_ERR("log_cfg malloc fail!\n");
		hal_mutex_unlock(tuning->mutex);
		return -1;
	}
	log_cfg->ae_log.ae_frame_id = isp_gen->ae_frame_cnt;
	log_cfg->ae_log.ev_sensor_exp_line = isp_gen->ae_entity_ctx.ae_result.sensor_set.ev_set_curr.ev_sensor_exp_line;
	log_cfg->ae_log.ev_exposure_time = isp_gen->ae_entity_ctx.ae_result.sensor_set.ev_set_curr.ev_exposure_time;
	log_cfg->ae_log.ev_analog_gain = isp_gen->ae_entity_ctx.ae_result.sensor_set.ev_set_curr.ev_analog_gain;
	log_cfg->ae_log.ev_digital_gain = isp_gen->ae_entity_ctx.ae_result.sensor_set.ev_set_curr.ev_digital_gain;
	log_cfg->ae_log.ev_total_gain = isp_gen->ae_entity_ctx.ae_result.sensor_set.ev_set_curr.ev_total_gain;
	log_cfg->ae_log.ev_idx = isp_gen->ae_entity_ctx.ae_result.sensor_set.ev_set_curr.ev_idx;
	log_cfg->ae_log.ev_idx_max = isp_gen->ae_entity_ctx.ae_result.sensor_set.ev_idx_max;
	log_cfg->ae_log.ev_lv = isp_gen->ae_entity_ctx.ae_result.sensor_set.ev_set_curr.ev_lv;
	log_cfg->ae_log.ev_lv_adj = isp_gen->ae_entity_ctx.ae_result.ev_lv_adj;
	log_cfg->ae_log.ae_target = isp_gen->ae_entity_ctx.ae_result.ae_target;
	log_cfg->ae_log.ae_avg_lum = isp_gen->ae_entity_ctx.ae_result.ae_avg_lum;
	log_cfg->ae_log.ae_weight_lum = isp_gen->ae_entity_ctx.ae_result.ae_weight_lum;
	log_cfg->ae_log.ae_delta_exp_idx = isp_gen->ae_entity_ctx.ae_result.ae_delta_exp_idx;
	log_cfg->ae_log.ev_sensor_exp_line_short = isp_gen->ae_entity_ctx.ae_result.sensor_set_short.ev_set_curr.ev_sensor_exp_line;
	log_cfg->ae_log.ev_exposure_time_short = isp_gen->ae_entity_ctx.ae_result.sensor_set_short.ev_set_curr.ev_exposure_time;
	log_cfg->ae_log.ev_analog_gain_short = isp_gen->ae_entity_ctx.ae_result.sensor_set_short.ev_set_curr.ev_analog_gain;
	log_cfg->ae_log.ev_digital_gain_short = isp_gen->ae_entity_ctx.ae_result.sensor_set_short.ev_set_curr.ev_digital_gain;
	log_cfg->ae_log.ratio_lm = isp_gen->ae_entity_ctx.ae_result.ae_wdr_ratio.real;
	log_cfg->ae_log.ae_mode = isp_gen->ae_settings.ae_mode;

	log_cfg->awb_log.awb_frame_id = isp_gen->awb_frame_cnt;
	log_cfg->awb_log.r_gain = isp_gen->awb_entity_ctx.awb_result.wb_gain_output.r_gain;
	log_cfg->awb_log.gr_gain = isp_gen->awb_entity_ctx.awb_result.wb_gain_output.gr_gain;
	log_cfg->awb_log.gb_gain = isp_gen->awb_entity_ctx.awb_result.wb_gain_output.gb_gain;
	log_cfg->awb_log.b_gain = isp_gen->awb_entity_ctx.awb_result.wb_gain_output.b_gain;
	log_cfg->awb_log.color_temp_output = isp_gen->awb_entity_ctx.awb_result.color_temp_output;
	log_cfg->awb_log.color_temp_target = isp_gen->awb_entity_ctx.awb_result.color_temp_target;
	for (i = 0; i < 10; i++) {
		log_cfg->awb_log.LightWinNum[i] = isp_gen->awb_entity_ctx.awb_result.LightWinNum[i];
		log_cfg->awb_log.LightTempMean[i] = isp_gen->awb_entity_ctx.awb_result.LightTempMean[i];
	}

	log_cfg->af_log.af_frame_id = isp_gen->af_frame_cnt;
	log_cfg->af_log.last_code_output = isp_gen->af_entity_ctx.af_result.last_code_output;
	log_cfg->af_log.real_code_output = isp_gen->af_entity_ctx.af_result.real_code_output;
	log_cfg->af_log.std_code_output = isp_gen->af_entity_ctx.af_result.std_code_output;

	log_cfg->iso_log.gain_idx = isp_gen->iso_entity_ctx.iso_result.gain_idx;
	log_cfg->iso_log.lum_idx = isp_gen->iso_entity_ctx.iso_result.lum_idx;
	log_cfg->iso_log.temp_enable = isp_gen->temp_info.enable;
	log_cfg->iso_log.temperature = isp_gen->sensor_info.temperature;
	for (i = 0; i < TEMP_COMP_MAX; i++) {
		log_cfg->iso_log.temperature_param[i] = isp_gen->temp_info.temperature_param[i];
	}

	log_cfg->pltm_log.pltm_auto_stren = isp_gen->pltm_entity_ctx.pltm_result.pltm_auto_stren;
	log_cfg->pltm_log.pltm_sharp_ss_compensation = isp_gen->pltm_entity_ctx.pltm_result.pltm_sharp_ss_compensation;
	log_cfg->pltm_log.pltm_sharp_ls_compensation = isp_gen->pltm_entity_ctx.pltm_result.pltm_sharp_ls_compensation;
	log_cfg->pltm_log.pltm_d2d_compensation = isp_gen->pltm_entity_ctx.pltm_result.pltm_d2d_compensation;
	log_cfg->pltm_log.pltm_d3d_compensation = isp_gen->pltm_entity_ctx.pltm_result.pltm_d3d_compensation;
	log_cfg->pltm_log.pltm_dark_block_num = isp_gen->pltm_entity_ctx.pltm_result.pltm_dark_block_num;
	log_cfg->pltm_log.cur_pic_lum = isp_gen->pltm_entity_ctx.pltm_result.cur_pic_lum;
	log_cfg->pltm_log.tar_pic_lum = isp_gen->pltm_entity_ctx.pltm_result.tar_pic_lum;
	memcpy(char_ptr, log_cfg, sizeof(struct isp_log_cfg));
	char_ptr += sizeof(struct isp_log_cfg);
	hal_free(log_cfg);
...
int isp_tuning_read_from_bin(struct hw_isp_device *isp, int *value)
{
	...
	if (flag == SET_BIN_TEST_3A_TUNING) {
		...
		memcpy(ctx->isp_cfg_name, &value[3], 50 * sizeof(char));
