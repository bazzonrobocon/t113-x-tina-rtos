				...
				struct scaler_para *para)
{
	...
	if (para->xratio == para->yratio) {
		...
		vin_log(VIN_LOG_SCALER, "para: xr = %u, yr = %u, w = %u, h = %u\n",
	...
	vin_log(VIN_LOG_SCALER, "crop: left = %d, top = %d, w = %u, h = %u\n",
...
int sunxi_scaler_subdev_s_stream(unsigned int id, int enable)
{
	...
	if (global_sensors[sensor_id].sensor_core->sensor_g_format)
		sensor_format = global_sensors[sensor_id].sensor_core->sensor_g_format(sensor_id, isp_sel, scaler->id);
	else
		return -1;
	...
	if (enable) {
		...
		hal_dcache_clean((unsigned long)scaler->load_para[0].phy_addr, VIPP_REG_SIZE);
	...
	vin_log(VIN_LOG_FMT, "vipp%d %s, %u*%u hoff: %d voff: %d xr: %u yr: %u\n",
...
static hal_irqreturn_t scaler_isr(void *priv)
{
	...
	if (vipp_status.chn0_reg_load_pd) {
		...
		if (scaler->load_select) {
			...
			hal_dcache_clean((unsigned long)scaler->load_para[1].phy_addr, VIPP_REG_SIZE);
	...
	if (vipp_status.chn1_reg_load_pd) {
		...
		if (global_scaler[scaler->id + 1]->load_select) {
			...
			hal_dcache_clean((unsigned long)global_scaler[scaler->id + 1]->load_para[1].phy_addr, VIPP_REG_SIZE);
			...
			hal_dcache_clean((unsigned long)global_scaler[scaler->id + 1]->load_para[0].phy_addr, VIPP_REG_SIZE);
	...
	if (vipp_status.chn2_reg_load_pd) {
		...
		if (global_scaler[scaler->id + 2]->load_select) {
			...
			hal_dcache_clean((unsigned long)global_scaler[scaler->id + 2]->load_para[1].phy_addr, VIPP_REG_SIZE);
			...
			hal_dcache_clean((unsigned long)global_scaler[scaler->id + 2]->load_para[0].phy_addr, VIPP_REG_SIZE);
	...
	if (vipp_status.chn3_reg_load_pd) {
		...
		if (global_scaler[scaler->id + 3]->load_select) {
			...
			hal_dcache_clean((unsigned long)global_scaler[scaler->id + 3]->load_para[1].phy_addr, VIPP_REG_SIZE);
			...
			hal_dcache_clean((unsigned long)global_scaler[scaler->id + 3]->load_para[0].phy_addr, VIPP_REG_SIZE);
...
static int scaler_resource_alloc(unsigned int id)
{
	...
	scaler->vipp_reg.phy_addr = memheap_alloc_align(&isp_mempool, scaler->vipp_reg.size, 0x1000);
#else
...
#if defined CONFIG_KERNEL_FREERTOS
	struct scaler_dev *scaler = global_scaler[id];
	memheap_free_align(scaler->vipp_reg.phy_addr);
#endif
}
#endif
...
int scaler_probe(unsigned int id)
{
	...
	if (global_sensors[sensor_id].sensor_core->sensor_g_format)
		sensor_format = global_sensors[sensor_id].sensor_core->sensor_g_format(sensor_id, isp_sel, id);
	else
		return -1;
