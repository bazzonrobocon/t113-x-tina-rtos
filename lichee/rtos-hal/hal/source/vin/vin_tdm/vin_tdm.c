...
static struct tdm_format *__tdm_try_format(unsigned int id)
{
	...
	int i;
	int sensor_sel = global_video[id].rear_sensor;
	int isp_sel = global_video[id].isp_sel;

	if (global_sensors[sensor_sel].sensor_core->sensor_g_format)
		sensor_format = global_sensors[sensor_sel].sensor_core->sensor_g_format(sensor_sel, isp_sel, id);
	else
		return NULL;
		...
		if (tdm_buf_num) {
			...
			if (size) {
				...
				for (i = 0; i < tdm_buf_num; i++) {
					...
					tdm_rx->buf[i].dma_addr = memheap_alloc_align(&isp_mempool, size, 0x1000);
#else
...
static int sunxi_tdm_top_s_stream(struct tdm_rx_dev *rx, int enable)
{
	...
	if (enable) {
		...
		vin_log(VIN_LOG_TDM, "%s rx mode %d, tx mode %d, work mode %u\n",
...
int sunxi_tdm_subdev_s_stream(unsigned int id, int vinc_id, int enable)
{
	...
	vin_log(VIN_LOG_FMT, "tdm_rx%d %s, %u*%u==%u*%u?\n",
...
int tdm_probe(unsigned int id)
{
	...
	vin_log(VIN_LOG_TDM, "tdm%d work mode is %u\n", tdm->id, tdm->work_mode);
...
#ifdef CONFIG_KERNEL_FREERTOS
				memheap_free_align(tdm_rx->buf[j].dma_addr);
#else
