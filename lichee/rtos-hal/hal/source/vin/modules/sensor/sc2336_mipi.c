...
static struct sensor_format_struct switch_sensor_formats[] = {
...
#if SENSOR_20FPS
	{
		...
		.gain_min   = 1 << 4,
		.gain_max   = 128 << 4,
		.switch_regs       = sensor_1080p20_regs,
		.switch_regs_size  = ARRAY_SIZE(sensor_1080p20_regs),
	},
#endif
...
static int sensor_reg_init(int id, int isp_id)
{
	...
	sc2336_sensor_vts = current_win[id]->vts;
	if (ispid == 0) {
		exp_gain.exp_val = clamp(*((unsigned int *)ISP0_NORFLASH_SAVE + 2), 16, sc2336_sensor_vts << 4);
		exp_gain.gain_val = clamp(*((unsigned int *)ISP0_NORFLASH_SAVE + 1), 16, 128 << 4);
	} else {
		exp_gain.exp_val = clamp(*((unsigned int *)ISP1_NORFLASH_SAVE + 2), 16, sc2336_sensor_vts << 4);
...
static int sensor_s_switch(int id)
{
	...
	sc2336_sensor_vts = current_switch_win[id]->vts;
...
struct sensor_fuc_core sc2336_core  = {
