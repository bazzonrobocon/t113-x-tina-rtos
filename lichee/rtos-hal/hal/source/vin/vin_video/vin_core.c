...
static hal_irqreturn_t vin_isr(void *priv)
{
	...
	if (status.vsync_trig) {
		...
		if (vinc->first_frame == 0) {
			vinc->first_frame = 1;
			goto set_next_buffer_addr;
		}

		if (list_empty(&vinc->vin_active) || vinc->vin_active.next->next == (&vinc->vin_active)) {
			vin_log(VIN_LOG_VIDEO, "No active queue to serve\n");
			goto vsync_end;
		}

		buffer = list_entry(vinc->vin_active.next, struct vin_buffer, list);
		list_move_tail(&buffer->list, &vinc->vin_done);

		if (list_empty(&vinc->vin_active)) {
			vin_log(VIN_LOG_VIDEO, "No more free frame\n");
			goto vsync_end;
		}
		if ((&vinc->vin_active) == vinc->vin_active.next->next) {
			vin_log(VIN_LOG_VIDEO, "No more free frame on next time\n");
			goto vsync_end;
		}
set_next_buffer_addr:
		buffer = list_entry(vinc->vin_active.next->next, struct vin_buffer, list);
		vin_set_addr(vinc->id, (unsigned long)buffer->phy_addr);


vsync_end:
		if (vinc->vin_buffer_process)
			vinc->vin_buffer_process(vinc->id);

		hal_wake_up(&vinc->vin_waitqueue);
...
int vin_core_probe(unsigned int id)
{
	...
	hal_waitqueue_head_init(&vinc->vin_waitqueue);
	INIT_LIST_HEAD(&vinc->vin_active);
	INIT_LIST_HEAD(&vinc->vin_done);
#endif
#ifdef DMA_USE_IRQ_BUFFER_QUEUE
#if defined CSIC_DMA_VER_140_000
	if (vinc->id == dma_virtual_find_logic[vinc->id]) {
...
#if defined CONFIG_VIDEO_SUNXI_VIN_SPECIAL
	hal_waitqueue_head_deinit(&vinc->vin_waitqueue);
#endif
