...
static int __csi_pin_config(unsigned int id, int enable)
{
	struct vin_csi_gpio_info *csi_gpio = &vin_csi_gpio[id];
	struct csi_dev *csi = global_csi[id];
	int i;

	if (csi->bus_info.bus_if == V4L2_MBUS_CSI2)
		return 0;

	if (enable) {
		for (i = 0; i < 11; i++) {
			hal_gpio_pinmux_set_function(csi_gpio->pin[i], csi_gpio->pin_func[0]);
		}
	} else {
		for (i = 0; i < 11; i++) {
			hal_gpio_pinmux_set_function(csi_gpio->pin[i], csi_gpio->pin_func[1]);
		}
	}
...
int sunxi_csi_subdev_s_stream(unsigned int id, unsigned int vinc_id, int enable)
{
	...
	if (enable) {
		...
		__csi_pin_config(csi->id, enable);
