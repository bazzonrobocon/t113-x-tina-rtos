...
static __s32 webcam_irq_handle(__u32 *addr0, __u32 *addr1, __u32 *addr2)
{
	...
	hal_log_info("pWebcamDevNode->pts_flag=%d pWebcamDevNode->pts=%ld\n",
...
__attribute__((__unused__)) static __s32 transport(__u32 addr0, __u32 addr1, __u32 addr2)
{
	UVCDev_t *UVCDev = NULL;

	UVCDev = g_usbWebcam->UVCDev;

	hal_log_info("usb:(%x,%x,%x),(%x,%x,%x), %px,%px\n", addr0, addr1, addr2,
		     UVCDev->frame_buf0, UVCDev->frame_buf1, UVCDev->frame_buf2,
		     UVCDev->active_buf, g_usbWebcam->uvc_irq);
	if (addr0 != 0) {
...
int DEV_WEBCAM_Ioctrl(struct devfs_node *node, int cmd, void *pbuffer)
{
	...
	case DRV_WEBCAM_CMD_CAPTURE_OFF: {
		if (pWebcamDevNode->tran_cb != NULL) {
			pWebcamDevNode->tran_cb((unsigned long)NULL, (unsigned long)NULL, (unsigned long)NULL);
		}

		pWebcamDevNode->capture = CAPTURE_NONE;
		pWebcamDevNode->used_addr0 = (unsigned long)NULL;
		pWebcamDevNode->used_addr1 = (unsigned long)NULL;
		// WEBCAM_capture_video_stop();//关中断发生器
		first_flag = 0;
		lost_frame_num = 0;

		hal_log_info("DRV_WEBCAM_CMD_CAPTURE_OFF pWebcamDevNode->tran_cb=%px\n",
			     pWebcamDevNode->tran_cb);
		ret = EPDK_OK;
	...
	case DRV_WENCAM_CMD_GET_IRQ_CB: {
		...
		ret = EPDK_OK;
		*cb = pWebcamDevNode->irq_cb;
		hal_log_err("DRV_WENCAM_CMD_GET_IRQ_CB[%px]\n", pbuffer);
		break;
	}
	case DRV_WENCAM_CMD_SET_TRANSPORT_CB: {
		hal_log_info("DRV_WENCAM_CMD_SET_TRANSPORT_CB[%px]\n", pbuffer);
		pWebcamDevNode->tran_cb = (transport_cb_t)pbuffer;
		ret = EPDK_OK;
		hal_log_err("DRV_WENCAM_CMD_SET_TRANSPORT_CB[%px]\n", pbuffer);
		break;
	}
	case DRV_WENCAM_CMD_WEBCAM_INSERT: {
		__s32 frame_id;
		...
		if (pWebcamDevNode->capture == CAPTURE_ON && pWebcamDevNode->tran_cb != NULL) {
			...
			if (frame_id != -1) {  //在free中请求buffer成功
			   ...
			   && pWebcamDevNode->used_addr0 != (unsigned long)NULL) {
			hal_log_info("DRV_WENCAM_CMD_WEBCAM_INSERT 2\n");
			pWebcamDevNode->tran_cb(pWebcamDevNode->used_addr0, (unsigned long)NULL,
						(unsigned long)NULL);
			pWebcamDevNode->capture = CAPTURE_ON_AND_CALLBACK;
		}
		hal_log_err("DRV_WENCAM_CMD_WEBCAM_INSERT\n");
