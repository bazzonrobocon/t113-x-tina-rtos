...
__s32 uvc_v4l2_do_ioctl(struct uvc_fh *handle, unsigned int cmd, void *arg)
{
	...
	case VIDIOC_QUERYCAP:  // 2.取得设备的capability，看看设备具有什么功能，比如是否具有视频输入,或者音频输入输出等。
	{
		...
		uvc_strlcpy((char *)cap->driver, "uvcvideo", sizeof cap->driver);
		uvc_strlcpy((char *)cap->card, "sunxi", sizeof cap->card);
	...
	case VIDIOC_ENUM_FMT: {
		...
		uvc_strlcpy((char *)fmt->description, format->name, sizeof fmt->description);
