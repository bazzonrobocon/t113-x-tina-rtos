...
__s32 usbWebcam_probe(UVCDev_t *UVCDev)
{
	...
	if (UVCDev == NULL || g_usbWebcam == NULL) {
		hal_log_err("ERR: usbWebcamProbe: input error, 0x%px,0x%px\n",
			    UVCDev, g_usbWebcam);
		return -1;
	}
...
void *usbWebcam_init(void)
{
	...
	g_usbWebcam = usbWebcam;
	{
		...
		for (i = 0; i < UVC_URBS; ++i) {
			usbWebcam->urb_mem[i] = (unsigned long)hal_malloc(UVC_MAX_PACKETS * 3);
...
__s32 usbWebcam_remove(UVCDev_t *UVCDev)
{
	...
	if (usbWebcam->webcamHdl) {
		...
		DEV_WEBCAM_Ioctrl(usbWebcam->webcamHdl, DRV_WENCAM_CMD_WEBCAM_MOVED,
				  (__s32)UVCDev->frame_buf0, (void *)(unsigned long)(UVCDev->frame_buf1));
	}
...
__s32 usbWebcam_exit(void *arg)
{
...

	{
		...
		for (i = 0; i < UVC_URBS; ++i) {
			if (usbWebcam->urb_mem[i]) {
				hal_free((void*)(unsigned long)usbWebcam->urb_mem[i]);
