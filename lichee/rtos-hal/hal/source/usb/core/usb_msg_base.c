...
static void _urb_timeout_process(void *data)
{
	...
	__err("ERR: _urb_timeout_process: time out urb = %p\n", urb);
	usb_unlink_urb(urb);
	return;
}
...
static int usb_start_wait_urb(struct urb *urb, int timeout_ms, int *actual_length)
{
	...
	if (status == 0) {
		...
		if (ret != 0) {
			hal_log_err("ERR: timed out on ep%d%s-%s len=%d/%d status:%d, flag:%d, setup:%lx, transfer:%p\n",
						...
						urb->transfer_flags, (long)urb->setup_dma, urb);
			status = -ETIMEDOUT;
		} else {
			status = urb->status;
		}
