...
void _debug_print_urb(struct urb *urb_input)
{
	//__inf(" = %x",urb_input-> );
	__inf("==print_urb=========================");
	__inf("urb addr         = %p", urb_input);
	__inf("lock_urb         = %p", (void *)&urb_input->lock_urb);
	__inf("wait_urb_timer   = %p", urb_input->wait_urb_timer);
	__inf("hcpriv           = %p", urb_input->hcpriv);
	__inf("bandwidth        = %x", urb_input->bandwidth);
	__inf("use_count        = %x", urb_input->use_count);
	__inf("reject           = %x", urb_input->reject);
	__inf("dev              = %p", urb_input->dev);
	__inf("pipe             = %x", urb_input->pipe);
	__inf("status           = %x", urb_input->status);
	__inf("transfer_flags   = %x", urb_input->transfer_flags);
	__inf("transfer_buffer  = %p", urb_input->transfer_buffer);
	__inf("transfer_dma     = %p", (void *)urb_input->transfer_dma);
	__inf("transfer_buffer_length = %x", urb_input->transfer_buffer_length);
	__inf("actual_length    = %x", urb_input->actual_length);
	__inf("setup_packet     = %p", urb_input->setup_packet);
	__inf("setup_dma        = %p", (void *)urb_input->setup_dma);
	...
	__inf("context          = %p", urb_input->context);
	__inf("complete         = %p", urb_input->complete);
