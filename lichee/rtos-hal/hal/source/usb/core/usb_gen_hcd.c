...
int hcd_ops_unlink_urb(struct urb *urb, int status)
{
	...
	if (!hcd->saw_irq && hcd->self.root_hub != urb->dev) {
		hal_log_err("WRN: Unlink after no-IRQ?  "
			    ...
			    "");
		hal_log_err("WRN: saw_irq = %d, root_hub = %p, dev = %p",
			    hcd->saw_irq, (void *)hcd->self.root_hub, (void *)urb->dev);
		hcd->saw_irq = 1;
	}
...
void hcd_ops_endpoint_disable(struct usb_device *udev, struct usb_host_endpoint *ep)
{
	...
	if (udev == NULL || (udev->bus == NULL) || ep == NULL) {
		hal_log_err("PANIC : hcd_ops_endpoint_disable() :input :udev = %p ,bus = %p ,ep  == %p ",
					(void *)udev, udev ? (udev->bus) : (NULL), (void *)ep);
		return;
	}
	...
	*/
	...
	cpu_sr = hal_spin_lock_irqsave(&gen_hcd_lock);
rescan:
	list_for_each(list_now, &ep->urb_list)
	{
