...
static int usb_phy_range_vcbus_write(unsigned long phy_vbase, int data)
{
	...
	/* set pre-emphasis*/
	bit_cal(data, USB_PHY_RANGE_PREEMPAMP, (int *)&value, (int *)&len);
...
// fix unused-function
__attribute__((__unused__)) static int usb_phy_range_vcbus_read(unsigned long phy_vbase)
{
...
void usb_phy_init(unsigned long phy_vbase, unsigned int usbc_no)
{
	...
	printf("akira calibration finish, val:0x%x, usbc_no:%d\n",
