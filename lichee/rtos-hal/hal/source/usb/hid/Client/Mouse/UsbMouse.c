...
static int  usbMouseOpen(struct devfs_node *node)
{
	usbMouse_t *usbMouse = NULL;

	usbMouse = (usbMouse_t *)(node->private);
...
static int usbMouseClose(struct devfs_node *node)
{
	usbMouse_t *usbMouse = NULL;
...
static ssize_t usbMouseRead(struct devfs_node *node, uint32_t addr, uint32_t size, void *data)
{
	hal_log_info("war: usbMouseRead not support\n");
	return 0;
}


static ssize_t usbMouseWrite(struct devfs_node *node, uint32_t addr, uint32_t size, const void *data)
{
	hal_log_info("war: usbMouseWrite not support\n");
	return 0;
}

static int32_t usbMouseIoctl(struct devfs_node *node, int cmd, void *arg)
{
	usbMouse_t *usbMouse = NULL;
	HidDev_t *HidDev = NULL;
	...
	usbMouse = (usbMouse_t *)(node->private);
	if (usbMouse->Magic != USB_HID_DEV_MAGIC) {
		hal_log_err("ERR: usbMouseClose: hidDev Magic(%x) is invalid\n", usbMouse->Magic);
		return EPDK_FAIL;
	}
	...
	case USBH_HID_USER_CTRL_CMD_TEST_START: {
		hal_log_info("usb mouse test command: start-s\n");
		if (arg == NULL) {
			hal_log_err("ERR: Execute usb mouse test cmd START failed, for arg is NULL\n");
			return EPDK_FAIL;
		}

		// ENTER_CRITICAL(cup_sr);
		hal_interrupt_disable();
		usbMouse->USBHMouseTest = (USBHMouseTest_t *)arg;
		usbMouse->USBMouseTestFlag = 1;
		hal_interrupt_enable();
		// EXIT_CRITICAL(cup_sr);
	...
	case USBH_HID_USER_CTRL_CMD_REG:
		...
		//usbMouse->CallBack = (USBHMouse_CallBack)esKRNL_GetCallBack((__pCBK_t)(pBuffer));
		/* to do for freertos */
		hal_log_err("need to do for freertos !!\n");
...
static void usbMouseEvent(usbMouse_t *usbMouse)
{
	...
	case USB_HID_TRANSPORT_CANCEL_CMD:
		...
		return;
		// break;
	case USB_HID_TRANSPORT_DEVICE_RESET:
		goto next;
...
*/
...
#elif defined(CONFIG_KERNEL_FREERTOS)
		usbMouse->dev_node.open = usbMouseOpen;
		usbMouse->dev_node.close = usbMouseClose;
		usbMouse->dev_node.read = usbMouseRead;
		usbMouse->dev_node.write = usbMouseWrite;
		usbMouse->dev_node.ioctl = usbMouseIoctl;
		usbMouse->dev_node.name = "mouse";
		usbMouse->dev_node.alias = "event";
		usbMouse->dev_node.private = usbMouse;
		usbMouse->dev_node.size = 1;
#endif
	...
	usbMouse->MouseRegHdle = esDEV_DevReg((const char *)usbMouse->ClassName,
					      ...
					      (void *)usbMouse);
#elif defined(CONFIG_KERNEL_FREERTOS)
	if (devfs_add_node(&(usbMouse->dev_node))) {
		hal_log_err("ERR: HID Mouse device register failed.");
		return USB_ERR_UNKOWN_ERROR;
	}
	usbMouse->MouseRegHdle = &(usbMouse->dev_node);
#endif
...
#elif defined(CONFIG_KERNEL_FREERTOS)
	//printf("\n ERR: ADD TO DO FUNCTION!! need call app\n\n");
#endif
...
#elif defined(CONFIG_KERNEL_FREERTOS)
	devfs_del_node(usbMouse->MouseRegHdle);
#endif
	}
