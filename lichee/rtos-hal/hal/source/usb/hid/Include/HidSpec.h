...
typedef struct _usbHidItem {
	...
	union {
		...
		signed char Data_s8;		   /* 有符号8位数 */
		unsigned short Data_u16;
		signed short Data_s16;
...
typedef struct _usbHidLocalItems {
...
typedef struct _usbHidUsage {
	...
	signed char hat_min;		 /* hat switch fun */
	signed char hat_max;		 /* ditto */
	signed char hat_dir;		 /* ditto */
} usbHidUsage_t;
