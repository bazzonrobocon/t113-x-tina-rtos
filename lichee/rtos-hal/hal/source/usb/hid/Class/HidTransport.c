...
static int HidUsbTransport(HidDev_t *HidDev)
{
	...
	if (ret == 0) {
		...
		hal_log_err("ERR: device is not present, CurrentUrb = %px\n", HidDev->CurrentUrb);
		HidDev->busy = 0;

		return USB_ERR_IO_DEVICE_OFFLINE;
	} else {
		hal_log_err("ERR: submit urb failed. ret = %d, CurrentUrb = %px\n", ret, HidDev->CurrentUrb);
		HidDev->busy = 0;
...
static int HidPortReset(HidDev_t *HidDev)
{
	...
	if (HidDev == NULL) {
		hal_log_err("ERR: HidPortReset: input error, mscDev = %px\n", HidDev);
		return USB_ERR_BAD_ARGUMENTS;
	}
...
{
	if (HidDev == NULL) {
		hal_log_err("ERR: HidStopTransport: input error, HidDev = %px\n", HidDev);
		return USB_ERR_BAD_ARGUMENTS;
	}
