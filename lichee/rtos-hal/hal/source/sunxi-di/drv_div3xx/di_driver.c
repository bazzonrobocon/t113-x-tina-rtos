...
#define SUNXI_IRQ_DI MAKE_IRQn(101,1)
...
int di_drv_client_dec(struct di_client *c)
{
	...
	if (drvdata->client_cnt == 0) {
		hal_disable_irq(drvdata->irq_no);
		hal_free_irq(drvdata->irq_no);
		di_clk_disable(drvdata);
		hal_reset_control_put(drvdata->rst_bus_di);
		di_mutex_unlock(drvdata->mlock);
		hal_sem_delete(drvdata->mlock);
		hal_free(drvdata);
		di_drvdata = NULL;
		return 0;
	}
	...
	struct di_driver_data *drvdata, struct di_client *c)
{
	...
	DI_INFO("TASK COUNT %d %p \n", drvdata->task_cnt, drvdata);
	ret = hal_wait_event_timeout(c->wait,
...
static hal_irqreturn_t di_irq_handler(void *dev_id)
{
	...
	hal_spin_lock(&drvdata->queue_lock);
...
irq_out:
	hal_spin_unlock(&drvdata->queue_lock);
	return HAL_IRQ_OK;
}
...
static int di_parse_dt(struct di_driver_data *drvdata)
{
	...
	drvdata->clk_source = hal_clock_get(clk_parent, CLK_PLL_PERI0_400M);
	...
	if (ret < 0) {
		DI_ERR("devm_request_irq failed %d\n", ret);
		goto err_out;
	}
...
int di_probe(void)
{
	...
	drvdata->mlock = hal_sem_create(1);

	DI_INFO("INIT CLIENT LIST \n");
