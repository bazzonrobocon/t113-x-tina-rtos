...
static void di_dev_set_md_buf(struct di_client *c)
{
	...
	u32 h_in_addr, h_out_addr;

	in_buf = c->md_buf.mbuf[c->md_buf.dir & 1];
	c->md_buf.dir ^= 1;
	out_buf = c->md_buf.mbuf[c->md_buf.dir & 1];
	reg->in_flag_laddr = __va_to_pa((unsigned long)(in_buf->vir_addr));
	reg->out_flag_laddr = __va_to_pa((unsigned long)(out_buf->vir_addr));
	h_in_addr = ((u32)(in_buf->vir_addr) >> 16);
	h_in_addr = (h_in_addr >> 16);
	reg->flag_haddr.bits.in_addr = h_in_addr;
		//(u32)(__va_to_pa((unsigned long)(in_buf->vir_addr)) >> 32);
	h_out_addr = ((u32)(out_buf->vir_addr) >> 16);
	h_out_addr = (h_out_addr >> 16);
	reg->flag_haddr.bits.out_addr = h_out_addr;
		//(u32)(__va_to_pa((unsigned long)(in_buf->vir_addr)) >> 32);
}
...
static s32 di_dev_set_md_para(struct di_client *c)
{
	...
	reg->md_cropv.dwval = (c->md_out_crop.top & 0xfffffffc) |
		...
		<< 16); */
	DI_DEBUG("left %d, right %d, top %d, bottom %d \n",
				c->md_out_crop.left,
				c->md_out_crop.right,
				c->md_out_crop.top,
				c->md_out_crop.bottom);
...
s32 di_dev_save_spot(void *client)
{
	...
	if (cdata->vof_buf == NULL) {
		cdata->vof_buf = hal_malloc(
			sizeof(*(cdata->vof_buf)));
		if (!cdata->vof_buf) {
			DI_ERR(TAG"malloc for vof_buf failed, size=%d\n",
...
{
	volatile struct di_reg *reg = di_dev_get_reg_base();
	u32 reg_val = reg->int_ctl.dwval;

	if (en)
		reg_val |= irq_flag;
	else
		reg_val &= ~irq_flag;
    reg->int_ctl.dwval = reg_val;
...
void di_dev_dump_reg_value(void)
{
	...
	for (i = 0x0; i <= 0x38; i += 4) {
		if ((i % 16) == 0)
			DI_INFO("0x%08lx:\n", i);
		DI_INFO("0x%08x \n", *((unsigned int *)((unsigned long)addr + i)));
		if (((i + 4) % 16) == 0)
			DI_INFO("\n");
	}
	DI_INFO("\n");
...
s32 di_dev_get_tnrpara(struct tnr_module_param_t *data)
{
	...
	/* Get the height and width of video */
	val = reg->dit_demoh.dwval;
	left = val & 0x7FF;
	right = ((val >> 16) & 0x7FF) + 1;
	val = reg->dit_demov.dwval;
	top = val & 0x7FF;
