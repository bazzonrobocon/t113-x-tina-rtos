...
//#define RAW_STEAM_DATA_LEN (128*1024)
#define RAW_STEAM_DATA_LEN (64*1024)

#define RUN_IN_RV 1

#if RUN_IN_RV
#define IN_BUFFER_LEN 128*1024
char *input = (char *)0x44000000;
long long in_index = 0;
#endif

int rv_read(void *buffer, int size)
{
    long long buffer_remain = IN_BUFFER_LEN - in_index;
    ...
    memcpy(buffer, input+in_index, size);
    in_index += size;
    logd("rv_read size :%d, in_index:%lld\n", size, in_index);
    return size;
}
...
int RawParserProbe(RawParserContext* pCtx)
{
    ...
    int code = -1;
    int i=0, ret=0;
    int nProbeDataLen = 10 * 1024;//10 * 1024
    ...
    loge("[vdecoderDemo] go to read nProbeDataLen = %d", nProbeDataLen);
#if RUN_IN_RV
    in_index = 0;//tmp
    //rv_read(data, nProbeDataLen);
    memcpy(data, input+in_index, nProbeDataLen);
    in_index += nProbeDataLen;
    logd("check in_index:%lld, nProbeDataLen = %d\n", in_index, nProbeDataLen);
#else
...
static int supplyData(RawParserContext* pCtx)
{
    ...
    {
    logd("[vdecoderDemo] go to read pCtx->mBufManager.nBufferLen = %d", pCtx->mBufManager.nBufferLen);
#if RUN_IN_RV
        nRealLen = rv_read(pCtx->mBufManager.pStreamBuffer, pCtx->mBufManager.nBufferLen);
#else
        nRealLen = fread(pCtx->mBufManager.pStreamBuffer, 1, \
            pCtx->mBufManager.nBufferLen, pCtx->fd);
#endif
    logd("[vdecoderDemo] nRealLen = %d", nRealLen);
        if(nRealLen <= 0)
            return -1;
        else
        {
    ...
    logd("[vdecoderDemo] nLen = %d", nLen);
#if RUN_IN_RV
        nRealLen = rv_read(pCtx->mBufManager.pStreamBuffer + pCtx->mBufManager.nValidSize, nLen);
#else
...
static int RawParserPrefetch_H264(RawParserContext* pCtx, DataPacketT* pPkt)
{
    ...
    if(pCtx->mBufManager.nValidSize <= 0)
    {
        ...
        if(nRet == -1) {
            logw("[%s %d][warning]supplyData return -1\n", __func__, __LINE__);
            return -1;
        }
    }
...
find_startCode:
    ...
    if(nFindStartCode == 1)
    {
        ...
        if(nFindStartCode == 1 && ((pCtx->mBufManager.nCurIndex + i-1)>0))
        {
...
find_startCode:
    ...
    pPkt->length = nStreamDataLen;
    logd("pPkt->length = %ld",pPkt->length);
    return 0;
}
...
{
    int nRet = -1;
    logi("start prefetch------------->streamType:%d", pCtx->streamType);
