...
int nor_write(unsigned int addr, char *buf, unsigned int size)
{
    ...
    status = HAL_Flash_Write(0 , addr, (uint8_t *)buf, size);
    if (status != HAL_OK) {
        printf("write %d fail\n", 0);
        return status;
    }
...
int nor_read(unsigned int addr, char *buf, unsigned int size)
{
    ...
    status = HAL_Flash_Read(0 , addr, (uint8_t *)buf, size);
    if (status != HAL_OK) {
        printf("read %d fail\n", 0);
        return status;
    }
...
#if defined CONFIG_FLASH_POWER_DOWN_PROTECT || defined CONFIG_DRIVERS_SPINOR_CACHE
unlock:
    nor_unlock();
#endif
