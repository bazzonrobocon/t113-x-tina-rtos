...
static int cmd_g2d_test_iommu(int argc, const char **argv)
{
	int ret;
	void *buf1 = 0;
	...
	sunxi_enable_device_iommu(G2D_IOMMU_MASTER_ID, true);

	printf("hello g2d_test\n");
	ret = sunxi_g2d_open();
	if (ret) {
		printf("g2d open fail\n");
		sunxi_enable_device_iommu(G2D_IOMMU_MASTER_ID, false);
		return -1;
	}

	handle = ion_alloc(1280 * 720 * 4);
	if (!handle) {
	...
	info.dst_image_h.mode = 1;
	info.dst_image_h.alpha = 255;
	info.dst_image_h.laddr[0] = dma_addr;//phy_addr1;
	fb_width = info.dst_image_h.width;
	fb_height = info.dst_image_h.height;
	info.dst_image_h.use_phy_addr = 1;
...
FINSH_FUNCTION_EXPORT_ALIAS(cmd_g2d_test_iommu, __cmd_g2d_test_iommu, g2d_test);
#endif
