...
void usb_test_show_help(void)
{
	printf("\nUsage:\n"\
		...
		"\twork_mode : [0-stop,1-start],usb mouse test work mode \n"\
...
static int usb_test_command_mouse(int argc, const char **argv)
{
#if defined(CONFIG_HAL_TEST_MOUSE)
	return usb_test_cmd_mouse(argc, argv);
#else
	printf("ERR: Can't find command config!\n");
	return -1;
#endif
...
static int usb_test_command(int argc, const char **argv)
{
	...
	else if (!strcmp(argv[1], "mouse_test"))
		ret = usb_test_command_mouse(argc, argv);
